...
{
    [Header("Ground Pieces (left-to-right order)")]
    ...
    private void Awake()
    {
        ...
        {
            Debug.LogError("GroundScroller: Assign at least 2 ground pieces.", this);
            enabled = false;
            return;
        }
        ...
        if (mainCamera == null)
        {
            Debug.LogError("GroundScroller: No Main Camera found.", this);
            enabled = false;
            return;
        }
        ...
        for (int i = 0; i < pieces.Length; i++)
        {
            ...
            if (sr == null)
            {
                Debug.LogError("GroundScroller: Each piece needs a SpriteRenderer.", this);
                enabled = false;
                return;
            }
    ...
    private void Update()
    {
        ...
        // Find rightmost edge X (so we can wrap to the end cleanly)
        ...
        // Wrap any piece that has gone fully off to the left
        for (int i = 0; i < pieces.Length; i++)
        {
            ...
            if (rightSide < leftEdge - offscreenPadding)
            {
                ...
                rightmostEdge = newX + halfWidths[i]; // update rightmost for multiple wraps in one frame
            }
        }
    }
}
